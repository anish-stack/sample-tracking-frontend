...
const Cuttingworker = () => {
  ...
  const fetchDataForQc = async () => {
    try {
      ...
      console.log("PATTERN CUTTING",res.data.data)
      if (res.data.data.length === 0) {
        setCuttingworker("No work assigned for the trim department person.");
      } else {
...
console.log(status)
 ...
 {/* New column for Comment */}
        ...
        <div className="qc-message">{Cuttingworker}</div>
      )}
    </div>
  );
};
